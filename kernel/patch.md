...
1. /arch/x86/kernel/time.c ： 添加timer.h 头文件，调用my_timer_handler() ,在setup_default_timer_irq()函数体中打印字符串
2. /include/linux/start_kernel.h ： 声明my_start_kernel
3. /include/linux/timer.h ： 声明my_timer_handler
4. /init/main.c  ： 调用my_start_kernel()
5. /Makefile :  Makefile中添加mykernel目录
6. 创建mykernel目录并在目录下创建Makefile 添加链接文件
